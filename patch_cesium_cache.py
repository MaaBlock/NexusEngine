...
"""
    ...
    if insert_pos == -1:
        print(f"[{tag}] Could not find namespace Cesium3DTilesSelection")
        return
    ...
    if find_str not in content:
        print(f"[{tag}] Could not find pAssetAccessor->get")
        return
...
"""
    ...
    print(f"[{tag}] Successfully patched.")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python patch_cesium_cache.py <cesium_native_src_dir>")
        sys.exit(1)
