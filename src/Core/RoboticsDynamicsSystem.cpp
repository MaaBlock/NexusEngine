...
// ------------------------------------------------------------------
// 从 MuJoCo 世界坐标（Z-up）构建引擎世界矩阵（Y-up）
//
// MuJoCo 约定: Z = up, X = 前, Y = 左
// 引擎约定   : Y = up, Z = 后, X = 右
// 等价于对 MuJoCo 坐标做: engine(x,y,z) = mj(x, z, -y)
//
// 四元数格式:
//   MuJoCo xquat = (w, x, y, z)
//   引擎 TransformComponent.rotation = (x, y, z, w)
// ------------------------------------------------------------------
    ...
    const double* mjPos, const double* mjQuat)
{
    ...
    float py = (float) mjPos[2];   // Z → Y
    float pz = (float)-mjPos[1];  // -Y → Z

    // 四元数转换: MuJoCo (w,x,y,z) → 旋转同坐标轴
    float qw = (float) mjQuat[0];
    float qx = (float) mjQuat[1];
    float qy = (float) mjQuat[3];   // Z轴分量 → Y
    float qz = (float)-mjQuat[2];  // -Y轴分量 → Z
