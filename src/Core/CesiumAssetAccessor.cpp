...
std::shared_ptr<CesiumAsync::IAssetRequest> CesiumAssetAccessor::executeRequest(
    ...
    if (url.find("file://") == 0 || (url.find("http") != 0 && url.find("https") != 0)) {
        ...
        if (!m_cachePath.empty()) {
            ...
            // Sanitize subDirStr (replace characters like ':' with '_')
            ...
            if (!cacheKey.empty()) {
                ...
                // Also check std::hash for backward compatibility just in case it was generated on Linux before
                ...
                if (std::filesystem::exists(hashFile2) && !std::filesystem::exists(hashFile1)) {
                    cacheFilePath = hashFile2; // Fallback to current OS std::hash if the file exists
                }
            } else {
        ...
        if (!cacheFilePath.empty() && std::filesystem::exists(cacheFilePath)) {
            // Note: In strict offline mode, we load EVERYTHING from cache.
            // If offline, we just load it if it exists. If online, we update JSONs from network unless it's a binary.
            if (m_offlineMode || (!isJson)) {
                ...
                if (fileStatus.ok()) {
                    ...
                    // Content Type Guessing from extension
        ...
        // Fetch from Network if not loaded from cache
        if (!loadedFromCache) {
            auto pos = url.find("://");
