...
// Calculate capacities (increased to 1GB VBO, 512MB IBO for planetary scale streaming)
static constexpr uint64_t VBO_SIZE = 1024ULL * 1024ULL * 1024ULL; // 1 GB
static constexpr uint64_t IBO_SIZE = 512ULL * 1024ULL * 1024ULL;  // 512 MB
...
Status MeshManager::addMesh(const std::vector<float>& vertices, const std::vector<uint32_t>& indices, uint32_t& outVertexOffset, uint32_t& outIndexOffset) {
    ...
    if (!m_indexAllocator->allocate(iCount, outIndexOffset)) {
        m_vertexAllocator->free(outVertexOffset); // Rollback
