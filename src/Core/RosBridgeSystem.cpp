...
struct RosBridgeSystem::Impl {
    ...
    // std::unique_ptr<zmq::socket_t> subscriber; // For future commands
...
void RosBridgeSystem::publishReplicas(Registry& registry) {
    ...
    json j_state;
    j_state["timestamp"] = 0.0; // TODO: get real simulation time
    ...
    for (auto entity : view) {
        ...
        // Ensure format is [x,y,z,w] for external nodes
    ...
    if (!j_state["bodies"].empty()) {
        ...
        // Topic allows multiple streams, using "tf" for transform states
