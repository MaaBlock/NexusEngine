...
void EditorUIManager::floatPanel(const std::string& panelId, float x, float y) {
    ...
    // We NO LONGER RemoveChild and AppendChild mid-drag to prevent freezing! 
    // Absolute position CSS takes care of the floating correctly until dropped
    for (auto& p : m_panels) {
        if (p->getId() == panelId) {
            p->setDocked(false);
            p->setDockZone("");
...
void EditorUIManager::ProcessEvent(Rml::Event& event) {
    ...
    if (event.GetId() == Rml::EventId::Dragstart) {
        ...
        // 如果点击命中了 slidertrack（RmlUi Bug：滚动条轨道覆盖整个面板），
        // 则通过鼠标坐标手动查找被遮挡的 tree-node
        ...
        if (target->GetTagName() == "input") {
            ...
            if (key == Rml::Input::KI_RETURN || key == Rml::Input::KI_NUMPADENTER) {
                target->Blur(); // Force loss of focus, prompting a Change event natively
            }
        }
    }
}
...
void EditorUIManager::setupEventListeners() {
    ...
    // Support global events via document capture (Capture Phase)
...
void EditorUIManager::update(Scene* scene) {
    ...
    // Throttle UI update logic to every other frame to save CPU time
