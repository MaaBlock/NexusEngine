...
static Rml::Input::KeyIdentifier translateKey(SDL_Scancode scancode) {
    // Only map control & navigation keys to RmlUi.
    // Printable characters (A-Z, 0-9, symbols) are handled by SDL_TEXT_INPUT to support IMEs and proper layouts.
    switch (scancode) {
...
void VK_UIBridge::processSdlEvent(const SDL_Event& event) {
    ...
    switch(event.type) {
        ...
        case SDL_EVENT_TEXT_INPUT: {
            // Text events are safely piped via injectTextInput from the polling thread.
            break;
        }
    }
}
