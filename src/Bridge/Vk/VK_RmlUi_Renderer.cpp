...
VK_RmlUi_Renderer::VK_RmlUi_Renderer(IContext* context, IRenderer* renderer)
    ...
    // Create a white 1x1 texture for untextured geometry
    ImageData whiteData;
    whiteData.width = 1;
    whiteData.height = 1;
...
Status VK_RmlUi_Renderer::createPipeline(uint32_t width, uint32_t height) {
    ...
    // RmlUi Vertex format: float2 pos, 4xuint8 color, float2 uv
    ...
    // Push constants layout match
    // struct BindlessConstants { float2 translation; float2 windowSize; uint textureIndex; uint samplerIndex; };
    ...
    vk::Format colorAttachmentFormat = vk::Format::eB8G8R8A8Unorm; // Match swapchain 
...
void VK_RmlUi_Renderer::RenderGeometry(Rml::CompiledGeometryHandle geometry_handle, Rml::Vector2f translation, Rml::TextureHandle texture) {
    ...
    // Bind viewport explicitly for dynamic states
