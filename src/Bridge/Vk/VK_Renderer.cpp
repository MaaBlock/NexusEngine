...
Status VK_Renderer::createGraphicsPipeline() {
    // 1. Define Descriptor Layout (Bindless Ready)
    ...
    auto layoutResult = m_descriptorManager->createLayout(bindings, true); // Enable Bindless flags
...
void VK_Renderer::recordCommandBuffer(vk::CommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    // TODO: In a real scenario, we would bind the material's descriptor set here:
...
Status VK_Renderer::onResize(uint32_t width, uint32_t height) {
    ...
    // Command buffers are reset every frame in my implementation, so no need to recreate them
    // but the viewport/scissor are dynamic, so they will update next frame.
    return OkStatus();
}
...
Status VK_Renderer::renderFrame() {
    ...
    if (acquireResult == vk::Result::eErrorOutOfDateKHR) {
        return OkStatus(); // Handled by resize
